...
void Json2Type(const Json::Value& value, DrdsCreateTableResponseType* item);
...
int Drds::CreateTable(const DrdsCreateTableRequestType& req,
                      DrdsCreateTableResponseType* response,
  ...
  if(!req.ddl_sql.empty()) {
    req_rpc->AddRequestQuery("DdlSql", req.ddl_sql);
  }
  if(!req.shard_type.empty()) {
    req_rpc->AddRequestQuery("ShardType", req.shard_type);
  }
  if(!req.shard_key.empty()) {
    req_rpc->AddRequestQuery("ShardKey", req.shard_key);
  }
