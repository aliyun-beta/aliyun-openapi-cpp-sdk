...
void Json2Type(const Json::Value& value, DrdsModifyDrdsIpWhiteListResponseType* item);
...
int Drds::ModifyDrdsIpWhiteList(const DrdsModifyDrdsIpWhiteListRequestType& req,
                      DrdsModifyDrdsIpWhiteListResponseType* response,
  ...
  if(!req.ip_white_list.empty()) {
    req_rpc->AddRequestQuery("IpWhiteList", req.ip_white_list);
  }
  if(!req.mode.empty()) {
