...
void Json2Type(const Json::Value& value, SecureCheckInfoType* item);
void Json2Type(const Json::Value& value, SecureCheckResponseType* item);
...
void Json2Type(const Json::Value& value, SecureCheckResponseType* item) {
  ...
  if(value.isMember("ProblemList") && value["ProblemList"].isMember("Info")) {
    Json2Array<SecureCheckInfoType>(value["ProblemList"]["Info"], &item->problem_list);
  }
  if(value.isMember("NoProblemList") && value["NoProblemList"].isMember("Info")) {
    Json2Array<SecureCheckInfoType>(value["NoProblemList"]["Info"], &item->no_problem_list);
  }
  if(value.isMember("NoScanList") && value["NoScanList"].isMember("Info")) {
    Json2Array<SecureCheckInfoType>(value["NoScanList"]["Info"], &item->no_scan_list);
  }
  if(value.isMember("ScanningList") && value["ScanningList"].isMember("Info")) {
    Json2Array<SecureCheckInfoType>(value["ScanningList"]["Info"], &item->scanning_list);
  }
  if(value.isMember("InnerIpList") && value["InnerIpList"].isMember("Info")) {
    Json2Array<SecureCheckInfoType>(value["InnerIpList"]["Info"], &item->inner_ip_list);
  }
}
}

int Yundun::SecureCheck(const SecureCheckRequestType& req,
                      SecureCheckResponseType* response,
