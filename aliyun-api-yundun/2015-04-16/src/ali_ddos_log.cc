...
void Json2Type(const Json::Value& value, DdosLogDdosLogType* item);
void Json2Type(const Json::Value& value, DdosLogResponseType* item);
...
void Json2Type(const Json::Value& value, DdosLogResponseType* item) {
  ...
  if(value.isMember("LogList") && value["LogList"].isMember("DdosLog")) {
    Json2Array<DdosLogDdosLogType>(value["LogList"]["DdosLog"], &item->log_list);
  }
}
}

int Yundun::DdosLog(const DdosLogRequestType& req,
                      DdosLogResponseType* response,
