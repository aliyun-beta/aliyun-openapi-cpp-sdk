...
void Json2Type(const Json::Value& value, YundunBruteforceLogBruteforceLogType* item);
void Json2Type(const Json::Value& value, YundunBruteforceLogResponseType* item);
...
void Json2Type(const Json::Value& value, YundunBruteforceLogResponseType* item) {
  ...
  if(value.isMember("LogList") && value["LogList"].isMember("BruteforceLog")) {
    Json2Array<YundunBruteforceLogBruteforceLogType>(value["LogList"]["BruteforceLog"], &item->log_list);
  }
}
}

int Yundun::BruteforceLog(const YundunBruteforceLogRequestType& req,
                      YundunBruteforceLogResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","BruteforceLog");
