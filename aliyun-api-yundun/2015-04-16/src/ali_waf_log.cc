...
void Json2Type(const Json::Value& value, WafLogWafLogType* item);
void Json2Type(const Json::Value& value, WafLogResponseType* item);
...
void Json2Type(const Json::Value& value, WafLogResponseType* item) {
  ...
  if(value.isMember("LogList") && value["LogList"].isMember("WafLog")) {
    Json2Array<WafLogWafLogType>(value["LogList"]["WafLog"], &item->log_list);
  }
}
}

int Yundun::WafLog(const WafLogRequestType& req,
                      WafLogResponseType* response,
