...
void Json2Type(const Json::Value& value, SlbSetLoadBalancerTCPListenerAttributeResponseType* item);
...
int Slb::SetLoadBalancerTCPListenerAttribute(const SlbSetLoadBalancerTCPListenerAttributeRequestType& req,
                      SlbSetLoadBalancerTCPListenerAttributeResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","SetLoadBalancerTCPListenerAttribute");
  ...
  if(!req.persistence_timeout.empty()) {
    req_rpc->AddRequestQuery("PersistenceTimeout", req.persistence_timeout);
  }
  ...
  if(!req.health_check_connect_timeout.empty()) {
    req_rpc->AddRequestQuery("HealthCheckConnectTimeout", req.health_check_connect_timeout);
  }
  ...
  if(!req.health_check_type.empty()) {
    req_rpc->AddRequestQuery("HealthCheckType", req.health_check_type);
  }
  if(!req.syn_proxy.empty()) {
    req_rpc->AddRequestQuery("SynProxy", req.syn_proxy);
  }
