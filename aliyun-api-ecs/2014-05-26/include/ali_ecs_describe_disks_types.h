#ifndef ALI_ECS_DESCRIBE_DISKS_TYPESH
#define ALI_ECS_DESCRIBE_DISKS_TYPESH
#include <stdio.h>
#include <string>
#include <vector>
namespace aliyun {
struct EcsDescribeDisksRequestType {
  std::string owner_id;
...
struct EcsDescribeDisksOperationLockType {
  std::string lock_reason;
};
struct EcsDescribeDisksTagType {
  std::string tag_key;
  std::string tag_value;
};
struct EcsDescribeDisksDiskType {
  std::vector<EcsDescribeDisksOperationLockType> operation_locks;
  std::vector<EcsDescribeDisksTagType> tags;
  std::string disk_id;
...
struct EcsDescribeDisksResponseType {
  std::vector<EcsDescribeDisksDiskType> disks;
  int total_count;
  int page_number;
  int page_size;
};
