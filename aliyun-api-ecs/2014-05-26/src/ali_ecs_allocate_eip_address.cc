...
void Json2Type(const Json::Value& value, EcsAllocateEipAddressResponseType* item);
...
void Json2Type(const Json::Value& value, EcsAllocateEipAddressResponseType* item) {
  ...
  if(value.isMember("EipAddress")) {
    item->eip_address = value["EipAddress"].asString();
  }
}
}

int Ecs::AllocateEipAddress(const EcsAllocateEipAddressRequestType& req,
                      EcsAllocateEipAddressResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","AllocateEipAddress");
