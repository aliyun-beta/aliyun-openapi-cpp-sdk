...
void Json2Type(const Json::Value& value, DescribeTagsTagType* item);
void Json2Type(const Json::Value& value, DescribeTagsResponseType* item);
...
void Json2Type(const Json::Value& value, DescribeTagsResponseType* item) {
  ...
  if(value.isMember("Tags") && value["Tags"].isMember("Tag")) {
    Json2Array<DescribeTagsTagType>(value["Tags"]["Tag"], &item->tags);
  }
}
}

int Ecs::DescribeTags(const DescribeTagsRequestType& req,
