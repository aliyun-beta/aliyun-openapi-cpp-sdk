...
void Json2Type(const Json::Value& value, EcsAttachDiskResponseType* item);
...
int Ecs::AttachDisk(const EcsAttachDiskRequestType& req,
                      EcsAttachDiskResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","AttachDisk");
  ...
  if(!req.device.empty()) {
    req_rpc->AddRequestQuery("Device", req.device);
  }
