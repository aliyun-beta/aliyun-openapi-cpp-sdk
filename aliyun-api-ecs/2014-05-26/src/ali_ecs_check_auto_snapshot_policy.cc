...
void Json2Type(const Json::Value& value, EcsCheckAutoSnapshotPolicyResponseType* item);
...
void Json2Type(const Json::Value& value, EcsCheckAutoSnapshotPolicyResponseType* item) {
  ...
  if(value.isMember("IsPermittedModify")) {
    item->is_permitted_modify = value["IsPermittedModify"].asString();
  }
}
}

int Ecs::CheckAutoSnapshotPolicy(const EcsCheckAutoSnapshotPolicyRequestType& req,
                      EcsCheckAutoSnapshotPolicyResponseType* response,
