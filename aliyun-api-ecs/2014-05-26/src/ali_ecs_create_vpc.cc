...
void Json2Type(const Json::Value& value, EcsCreateVpcResponseType* item);
...
int Ecs::CreateVpc(const EcsCreateVpcRequestType& req,
                      EcsCreateVpcResponseType* response,
  ...
  if(!req.vpc_name.empty()) {
    req_rpc->AddRequestQuery("VpcName", req.vpc_name);
  }
