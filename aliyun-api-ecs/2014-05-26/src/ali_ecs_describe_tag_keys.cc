...
void Json2Type(const Json::Value& value, EcsDescribeTagKeysResponseType* item);
...
void Json2Type(const Json::Value& value, EcsDescribeTagKeysResponseType* item) {
  ...
  if(value.isMember("TagKeys")  && value["TagKeys"].isMember("TagKey")) {
    Json2Array<std::string>(value["TagKeys"]["TagKey"], &item->tag_keys);
  }
}
}

int Ecs::DescribeTagKeys(const EcsDescribeTagKeysRequestType& req,
                      EcsDescribeTagKeysResponseType* response,
