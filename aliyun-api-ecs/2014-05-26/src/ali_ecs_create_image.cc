...
void Json2Type(const Json::Value& value, EcsCreateImageResponseType* item);
...
int Ecs::CreateImage(const EcsCreateImageRequestType& req,
                      EcsCreateImageResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","CreateImage");
  ...
  if(!req.image_version.empty()) {
    req_rpc->AddRequestQuery("ImageVersion", req.image_version);
  }
