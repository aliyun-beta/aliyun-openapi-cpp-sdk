...
void Json2Type(const Json::Value& value, DescribeInstancesOcsInstanceType* item);
...
void Json2Type(const Json::Value& value, DescribeInstancesResponseType* item) {
  ...
  if(value.isMember("Instances") && value["Instances"].isMember("OcsInstance")) {
    Json2Array<DescribeInstancesOcsInstanceType>(value["Instances"]["OcsInstance"], &item->instances);
  }
}
}

int Ocs::DescribeInstances(const DescribeInstancesRequestType& req,
