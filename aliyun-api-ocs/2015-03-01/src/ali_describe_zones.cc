...
void Json2Type(const Json::Value& value, DescribeZonesOcsZoneType* item);
...
void Json2Type(const Json::Value& value, DescribeZonesResponseType* item) {
  if(value.isMember("Zones") && value["Zones"].isMember("OcsZone")) {
    Json2Array<DescribeZonesOcsZoneType>(value["Zones"]["OcsZone"], &item->zones);
  }
}
}

int Ocs::DescribeZones(const DescribeZonesRequestType& req,
