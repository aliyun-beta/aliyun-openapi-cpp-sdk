...
void Json2Type(const Json::Value& value, DescribePriceRuleType* item);
void Json2Type(const Json::Value& value, DescribePriceOrderType* item);
void Json2Type(const Json::Value& value, DescribePriceResponseType* item);
...
void Json2Type(const Json::Value& value, DescribePriceRuleType* item) {
  if(value.isMember("RuleDescId")) {
    item->rule_desc_id = value["RuleDescId"].asInt();
  }
...
void Json2Type(const Json::Value& value, DescribePriceResponseType* item) {
  if(value.isMember("Rules") && value["Rules"].isMember("Rule")) {
    Json2Array<DescribePriceRuleType>(value["Rules"]["Rule"], &item->rules);
  }
  if(value.isMember("Order")) {
    Json2Type(value["Order"], &item->order);
  }
}
}

int Rkvstore::DescribePrice(const DescribePriceRequestType& req,
                      DescribePriceResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","DescribePrice");
  ...
  if(!req.order_type.empty()) {
    req_rpc->AddRequestQuery("OrderType", req.order_type);
  }
