...
void Json2Type(const Json::Value& value, AceopsqueryResponseType* item);
...
void Json2Type(const Json::Value& value, AceopsqueryResponseType* item) {
  ...
  if(value.isMember("softversion")) {
    item->softversion = value["softversion"].asString();
  }
}
}

int Aceops::query(const AceopsqueryRequestType& req,
                      AceopsqueryResponseType* response,
  ...
  if(!req.group.empty()) {
    req_rpc->AddRequestQuery("group", req.group);
  }
  if(!req.name.empty()) {
