...
void Json2Type(const Json::Value& value, CdnDescribeOneMinuteDataResponseType* item);
...
void Json2Type(const Json::Value& value, CdnDescribeOneMinuteDataResponseType* item) {
  if(value.isMember("Bps")) {
    item->bps = value["Bps"].asString();
  }
  if(value.isMember("Qps")) {
    item->qps = value["Qps"].asString();
  }
}
}

int Cdn::DescribeOneMinuteData(const CdnDescribeOneMinuteDataRequestType& req,
                      CdnDescribeOneMinuteDataResponseType* response,
