...
void Json2Type(const Json::Value& value, AlertUpdateAlertResponseType* item);
...
int Alert::UpdateAlert(const AlertUpdateAlertRequestType& req,
                      AlertUpdateAlertResponseType* response,
  ...
  std::string url = secheme  + "://" + host_ + get_format_string("/projects/%s/alerts/%s", req.project_name.c_str(), req.alert_name.c_str());
  ...
  if(req_rpc->CommitRequestWithBody(req.alert) != 0) {
     if(error_info) {
