...
void Json2Type(const Json::Value& value, AlertListDBSourceResponseType* item);
...
int Alert::ListDBSource(const AlertListDBSourceRequestType& req,
                      AlertListDBSourceResponseType* response,
  ...
  std::string url = secheme  + "://" + host_ + get_format_string("/projects/%s/sources", req.project_name.c_str());
  ...
  if(!req.source_name.empty()) {
    req_rpc->AddRequestQuery("SourceName", req.source_name);
  }
  if(!req.page.empty()) {
