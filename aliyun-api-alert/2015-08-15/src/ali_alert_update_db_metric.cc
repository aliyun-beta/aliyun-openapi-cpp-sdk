...
void Json2Type(const Json::Value& value, AlertUpdateDBMetricResponseType* item);
...
int Alert::UpdateDBMetric(const AlertUpdateDBMetricRequestType& req,
                      AlertUpdateDBMetricResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  std::string url = "http://" + host_ + get_format_string("/projects/%s/dbMetrics/%s", req.project_name.c_str(), req.metric_name.c_str());
