...
void Json2Type(const Json::Value& value, AlertCreateAlertResponseType* item);
...
int Alert::CreateAlert(const AlertCreateAlertRequestType& req,
                      AlertCreateAlertResponseType* response,
                      AlertErrorInfo* error_info) {
  std::string str_response;
  int status_code;
  int ret = 0;
  bool parse_success = false;
  std::string url = "http://" + host_ + get_format_string("/projects/%s/alerts", req.project_name.c_str());
