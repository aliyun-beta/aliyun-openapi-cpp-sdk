...
void Json2Type(const Json::Value& value, AlertUpdateContactGroupResponseType* item);
...
int Alert::UpdateContactGroup(const AlertUpdateContactGroupRequestType& req,
                      AlertUpdateContactGroupResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  std::string url = "http://" + host_ + get_format_string("/projects/%s/groups/%s", req.project_name.c_str(), req.group_name.c_str());
  AliRoaRequest* req_rpc = new AliRoaRequest(version_,
                         appid_,
                         secret_,
                         url);
  req_rpc->setRequestMethod("PUT");
  if(req_rpc->CommitRequestWithBody(req.contact_group) != 0) {
     if(error_info) {
