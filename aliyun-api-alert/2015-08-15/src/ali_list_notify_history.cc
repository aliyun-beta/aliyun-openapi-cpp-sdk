...
void Json2Type(const Json::Value& value, ListNotifyHistoryResponseType* item);
...
int Alert::ListNotifyHistory(const ListNotifyHistoryRequestType& req,
                      ListNotifyHistoryResponseType* response,
                      AliAlertErrorInfo* error_info) {
  std::string str_response;
  int status_code;
  int ret = 0;
  bool parse_success = false;
  std::string url = "http://" + host_ + get_format_string("/projects/%s/notify_history", req.project_name.c_str());
