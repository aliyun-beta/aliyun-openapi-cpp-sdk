...
void Json2Type(const Json::Value& value, AlertUpdateLogHubMetricResponseType* item);
...
int Alert::UpdateLogHubMetric(const AlertUpdateLogHubMetricRequestType& req,
                      AlertUpdateLogHubMetricResponseType* response,
  ...
  std::string url = secheme  + "://" + host_ + get_format_string("/projects/%s/logHubMetrics/%s", req.project_name.c_str(), req.metric_name.c_str());
  ...
  if(req_rpc->CommitRequestWithBody(req.metric) != 0) {
     if(error_info) {
