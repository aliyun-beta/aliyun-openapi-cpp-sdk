...
void Json2Type(const Json::Value& value, AlertRemoveProjectOwnerResponseType* item);
...
int Alert::RemoveProjectOwner(const AlertRemoveProjectOwnerRequestType& req,
                      AlertRemoveProjectOwnerResponseType* response,
  ...
  std::string url = secheme  + "://" + host_ + get_format_string("/projects/removeOwner");
  ...
  if(!req.owners.empty()) {
    req_rpc->AddRequestQuery("Owners", req.owners);
  }
