...
void Json2Type(const Json::Value& value, RdsRegionDescribeModifyParameterLogParameterChangeLogType* item);
void Json2Type(const Json::Value& value, RdsRegionDescribeModifyParameterLogResponseType* item);
...
void Json2Type(const Json::Value& value, RdsRegionDescribeModifyParameterLogParameterChangeLogType* item) {
  if(value.isMember("ModifyTime")) {
    item->modify_time = value["ModifyTime"].asString();
  }
  if(value.isMember("OldParameterValue")) {
    item->old_parameter_value = value["OldParameterValue"].asString();
  }
  if(value.isMember("NewParameterValue")) {
    item->new_parameter_value = value["NewParameterValue"].asString();
  }
...
void Json2Type(const Json::Value& value, RdsRegionDescribeModifyParameterLogResponseType* item) {
  ...
  if(value.isMember("Items") && value["Items"].isMember("ParameterChangeLog")) {
    Json2Array<RdsRegionDescribeModifyParameterLogParameterChangeLogType>(value["Items"]["ParameterChangeLog"], &item->items);
  }
}
}

int RdsRegion::DescribeModifyParameterLog(const RdsRegionDescribeModifyParameterLogRequestType& req,
                      RdsRegionDescribeModifyParameterLogResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","DescribeModifyParameterLog");
