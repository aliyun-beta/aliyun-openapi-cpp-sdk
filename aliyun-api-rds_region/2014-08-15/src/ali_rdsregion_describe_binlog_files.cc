...
void Json2Type(const Json::Value& value, RdsRegionDescribeBinlogFilesBinLogFileType* item);
void Json2Type(const Json::Value& value, RdsRegionDescribeBinlogFilesResponseType* item);
...
void Json2Type(const Json::Value& value, RdsRegionDescribeBinlogFilesResponseType* item) {
  ...
  if(value.isMember("Items") && value["Items"].isMember("BinLogFile")) {
    Json2Array<RdsRegionDescribeBinlogFilesBinLogFileType>(value["Items"]["BinLogFile"], &item->items);
  }
}
}

int RdsRegion::DescribeBinlogFiles(const RdsRegionDescribeBinlogFilesRequestType& req,
                      RdsRegionDescribeBinlogFilesResponseType* response,
