...
void Json2Type(const Json::Value& value, RdsRegionDescribeRealtimeDiagnosesRealtimeDiagnoseTasksType* item);
void Json2Type(const Json::Value& value, RdsRegionDescribeRealtimeDiagnosesResponseType* item);
...
void Json2Type(const Json::Value& value, RdsRegionDescribeRealtimeDiagnosesResponseType* item) {
  ...
  if(value.isMember("Tasks") && value["Tasks"].isMember("RealtimeDiagnoseTasks")) {
    Json2Array<RdsRegionDescribeRealtimeDiagnosesRealtimeDiagnoseTasksType>(value["Tasks"]["RealtimeDiagnoseTasks"], &item->tasks);
  }
}
}

int RdsRegion::DescribeRealtimeDiagnoses(const RdsRegionDescribeRealtimeDiagnosesRequestType& req,
                      RdsRegionDescribeRealtimeDiagnosesResponseType* response,
