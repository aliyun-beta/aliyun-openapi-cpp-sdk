...
void Json2Type(const Json::Value& value, RdsRegionDescribeParametersDBInstanceParameterType* item);
void Json2Type(const Json::Value& value, RdsRegionDescribeParametersResponseType* item);
...
void Json2Type(const Json::Value& value, RdsRegionDescribeParametersResponseType* item) {
  ...
  if(value.isMember("ConfigParameters") && value["ConfigParameters"].isMember("DBInstanceParameter")) {
    Json2Array<RdsRegionDescribeParametersDBInstanceParameterType>(value["ConfigParameters"]["DBInstanceParameter"], &item->config_parameters);
  }
  if(value.isMember("RunningParameters") && value["RunningParameters"].isMember("DBInstanceParameter")) {
    Json2Array<RdsRegionDescribeParametersDBInstanceParameterType>(value["RunningParameters"]["DBInstanceParameter"], &item->running_parameters);
  }
}
}

int RdsRegion::DescribeParameters(const RdsRegionDescribeParametersRequestType& req,
                      RdsRegionDescribeParametersResponseType* response,
