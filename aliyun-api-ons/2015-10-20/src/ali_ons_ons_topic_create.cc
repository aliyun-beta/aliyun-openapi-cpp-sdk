...
void Json2Type(const Json::Value& value, OnsOnsTopicCreateResponseType* item);
...
int Ons::OnsTopicCreate(const OnsOnsTopicCreateRequestType& req,
                      OnsOnsTopicCreateResponseType* response,
  ...
  if(!req.queue_num.empty()) {
    req_rpc->AddRequestQuery("QueueNum", req.queue_num);
  }
  if(!req.order.empty()) {
    req_rpc->AddRequestQuery("Order", req.order);
  }
  if(!req.qps.empty()) {
    req_rpc->AddRequestQuery("Qps", req.qps);
  }
  if(!req.status.empty()) {
    req_rpc->AddRequestQuery("Status", req.status);
  }
  if(!req.remark.empty()) {
    req_rpc->AddRequestQuery("Remark", req.remark);
  }
  if(!req.appkey.empty()) {
    req_rpc->AddRequestQuery("Appkey", req.appkey);
  }
