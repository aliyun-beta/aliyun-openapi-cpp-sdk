...
void Json2Type(const Json::Value& value, OnsOnsPublishGetPublishInfoDoType* item);
void Json2Type(const Json::Value& value, OnsOnsPublishGetResponseType* item);
...
void Json2Type(const Json::Value& value, OnsOnsPublishGetResponseType* item) {
  ...
  if(value.isMember("Data") && value["Data"].isMember("PublishInfoDo")) {
    Json2Array<OnsOnsPublishGetPublishInfoDoType>(value["Data"]["PublishInfoDo"], &item->data);
  }
}
}

int Ons::OnsPublishGet(const OnsOnsPublishGetRequestType& req,
                      OnsOnsPublishGetResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","OnsPublishGet");
  ...
  if(!req.producer_id.empty()) {
    req_rpc->AddRequestQuery("ProducerId", req.producer_id);
  }
  if(!req.topic.empty()) {
