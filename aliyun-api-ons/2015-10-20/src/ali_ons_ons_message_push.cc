...
void Json2Type(const Json::Value& value, OnsOnsMessagePushResponseType* item);
...
int Ons::OnsMessagePush(const OnsOnsMessagePushRequestType& req,
                      OnsOnsMessagePushResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","OnsMessagePush");
  ...
  if(!req.client_id.empty()) {
    req_rpc->AddRequestQuery("ClientId", req.client_id);
  }
  if(!req.msg_id.empty()) {
