...
void Json2Type(const Json::Value& value, OnsOnsEmpowerCreateResponseType* item);
...
int Ons::OnsEmpowerCreate(const OnsOnsEmpowerCreateRequestType& req,
                      OnsOnsEmpowerCreateResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","OnsEmpowerCreate");
  ...
  if(!req.relation.empty()) {
    req_rpc->AddRequestQuery("Relation", req.relation);
  }
