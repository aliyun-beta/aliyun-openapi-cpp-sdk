...
void Json2Type(const Json::Value& value, OnsOnsMessageSendResponseType* item);
...
void Json2Type(const Json::Value& value, OnsOnsMessageSendResponseType* item) {
  ...
  if(value.isMember("Data")) {
    item->data = value["Data"].asString();
  }
}
}

int Ons::OnsMessageSend(const OnsOnsMessageSendRequestType& req,
                      OnsOnsMessageSendResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","OnsMessageSend");
  ...
  if(!req.tag.empty()) {
    req_rpc->AddRequestQuery("Tag", req.tag);
  }
  if(!req.key.empty()) {
    req_rpc->AddRequestQuery("Key", req.key);
  }
  if(!req.message.empty()) {
    req_rpc->AddRequestQuery("Message", req.message);
  }
