...
void Json2Type(const Json::Value& value, RamUpdateUserUserType* item);
void Json2Type(const Json::Value& value, RamUpdateUserResponseType* item);
...
int Ram::UpdateUser(const RamUpdateUserRequestType& req,
                      RamUpdateUserResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","UpdateUser");
  if(!req.user_name.empty()) {
    req_rpc->AddRequestQuery("UserName", req.user_name);
  }
  if(!req.new_user_name.empty()) {
    req_rpc->AddRequestQuery("NewUserName", req.new_user_name);
  }
  if(!req.new_display_name.empty()) {
    req_rpc->AddRequestQuery("NewDisplayName", req.new_display_name);
  }
  if(!req.new_mobile_phone.empty()) {
    req_rpc->AddRequestQuery("NewMobilePhone", req.new_mobile_phone);
  }
  if(!req.new_email.empty()) {
    req_rpc->AddRequestQuery("NewEmail", req.new_email);
  }
  if(!req.new_comments.empty()) {
    req_rpc->AddRequestQuery("NewComments", req.new_comments);
  }
