...
void Json2Type(const Json::Value& value, ListEntitiesForPolicyGroupType* item);
void Json2Type(const Json::Value& value, ListEntitiesForPolicyUserType* item);
void Json2Type(const Json::Value& value, ListEntitiesForPolicyRoleType* item);
void Json2Type(const Json::Value& value, ListEntitiesForPolicyResponseType* item);
...
void Json2Type(const Json::Value& value, ListEntitiesForPolicyResponseType* item) {
  if(value.isMember("Groups") && value["Groups"].isMember("Group")) {
    Json2Array<ListEntitiesForPolicyGroupType>(value["Groups"]["Group"], &item->groups);
  }
  if(value.isMember("Users") && value["Users"].isMember("User")) {
    Json2Array<ListEntitiesForPolicyUserType>(value["Users"]["User"], &item->users);
  }
  if(value.isMember("Roles") && value["Roles"].isMember("Role")) {
    Json2Array<ListEntitiesForPolicyRoleType>(value["Roles"]["Role"], &item->roles);
  }
}
}

int Ram::ListEntitiesForPolicy(const ListEntitiesForPolicyRequestType& req,
                      ListEntitiesForPolicyResponseType* response,
