...
void Json2Type(const Json::Value& value, RiskSendVerifyCodeResponseType* item);
...
int Risk::SendVerifyCode(const RiskSendVerifyCodeRequestType& req,
                      RiskSendVerifyCodeResponseType* response,
  ...
  if(!req.message_parameters.empty()) {
    req_rpc->AddRequestQuery("MessageParameters", req.message_parameters);
  }
  if(!req.extend.empty()) {
