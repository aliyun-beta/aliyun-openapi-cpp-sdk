...
void Json2Type(const Json::Value& value, OssAdminPutBucketLimitResponseType* item);
...
int OssAdmin::PutBucketLimit(const OssAdminPutBucketLimitRequestType& req,
                      OssAdminPutBucketLimitResponseType* response,
  ...
  if(!req.bucket_limit.empty()) {
    req_rpc->AddRequestQuery("BucketLimit", req.bucket_limit);
  }
