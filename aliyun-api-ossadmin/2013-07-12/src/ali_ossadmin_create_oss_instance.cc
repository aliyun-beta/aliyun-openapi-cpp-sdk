...
void Json2Type(const Json::Value& value, OssAdmincreateOssInstanceResponseType* item);
...
void Json2Type(const Json::Value& value, OssAdmincreateOssInstanceResponseType* item) {
  ...
  if(value.isMember("aliUid")) {
    item->ali_uid = value["aliUid"].asInt();
  }
  if(value.isMember("instanceId")) {
    item->instance_id = value["instanceId"].asString();
  }
  if(value.isMember("instacneStatus")) {
    item->instacne_status = value["instacneStatus"].asString();
  }
...
int OssAdmin::createOssInstance(const OssAdmincreateOssInstanceRequestType& req,
                      OssAdmincreateOssInstanceResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","createOssInstance");
  ...
  if(!req.ali_uid.empty()) {
    req_rpc->AddRequestQuery("aliUid", req.ali_uid);
  }
