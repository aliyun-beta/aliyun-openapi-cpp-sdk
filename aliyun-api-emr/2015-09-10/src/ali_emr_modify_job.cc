...
void Json2Type(const Json::Value& value, EmrModifyJobResponseType* item);
...
int Emr::ModifyJob(const EmrModifyJobRequestType& req,
                      EmrModifyJobResponseType* response,
  ...
  if(!req.env_parameter.empty()) {
    req_rpc->AddRequestQuery("EnvParameter", req.env_parameter);
  }
