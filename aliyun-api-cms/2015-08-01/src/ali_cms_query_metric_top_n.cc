...
void Json2Type(const Json::Value& value, CmsQueryMetricTopNResponseType* item);
...
int Cms::QueryMetricTopN(const CmsQueryMetricTopNRequestType& req,
                      CmsQueryMetricTopNResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","QueryMetricTopN");
  ...
  if(!req.value_key.empty()) {
    req_rpc->AddRequestQuery("ValueKey", req.value_key);
  }
  if(!req.top.empty()) {
    req_rpc->AddRequestQuery("Top", req.top);
  }
  if(!req.extend.empty()) {
