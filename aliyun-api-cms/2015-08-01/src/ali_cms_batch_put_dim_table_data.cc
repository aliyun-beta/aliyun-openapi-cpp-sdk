...
void Json2Type(const Json::Value& value, CmsBatchPutDimTableDataResponseType* item);
...
int Cms::BatchPutDimTableData(const CmsBatchPutDimTableDataRequestType& req,
                      CmsBatchPutDimTableDataResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","BatchPutDimTableData");
  if(!req.dim_table_name.empty()) {
    req_rpc->AddRequestQuery("DimTableName", req.dim_table_name);
  }
  if(!req.body.empty()) {
    req_rpc->AddRequestQuery("Body", req.body);
  }
