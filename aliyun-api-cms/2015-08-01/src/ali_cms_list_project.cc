...
void Json2Type(const Json::Value& value, CmsListProjectResponseType* item);
...
int Cms::ListProject(const CmsListProjectRequestType& req,
                      CmsListProjectResponseType* response,
  ...
  Json::Value val;
  Json::Reader reader;
  req_rpc->AddRequestQuery("Action","ListProject");
  if(!req.project_owner.empty()) {
    req_rpc->AddRequestQuery("ProjectOwner", req.project_owner);
  }
  if(!req.page.empty()) {
