...
static const char* test_put_metric_datum_response = "{"
"  \"Code\": \"Code\","
"  \"Message\": \"Message\""
"}";
static int test_put_metric_datum() {
  CmsPutMetricDatumRequestType req;
  ...
  cms->SetUseTls(false);
  HttpTestListener* listener = new HttpTestListener(12234);
  listener->SetResponseBody(test_put_metric_datum_response);
  listener->Start();
  req.resource_owner_id = "ResourceOwnerId";
  req.namespace_ = "Namespace_";
  req.metrics = "Metrics";
  int ret = cms->PutMetricDatum(req, &resp, NULL);
  listener->WaitComplete();
  delete listener;
  delete cms;
}
