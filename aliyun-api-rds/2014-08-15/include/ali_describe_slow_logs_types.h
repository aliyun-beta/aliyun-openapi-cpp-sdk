#ifndef ALI_DESCRIBE_SLOW_LOGS_TYPESH
#define ALI_DESCRIBE_SLOW_LOGS_TYPESH
#include <stdio.h>
#include <string>
#include <vector>
namespace aliyun {
struct DescribeSlowLogsRequestType {
  std::string owner_id;
...
struct DescribeSlowLogsSQLSlowLogType {
  long slow_log_id;
  long sq_lid;
  std::string db_name;
  std::string sq_ltext;
...
struct DescribeSlowLogsResponseType {
  std::vector<DescribeSlowLogsSQLSlowLogType> items;
  std::string engine;
  std::string start_time;
  std::string end_time;
