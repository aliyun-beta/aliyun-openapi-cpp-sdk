#ifndef ALI_RDS_DESCRIBE_SLOW_LOGS_TYPESH
#define ALI_RDS_DESCRIBE_SLOW_LOGS_TYPESH
#include <stdio.h>
#include <string>
#include <vector>
namespace aliyun {
struct RdsDescribeSlowLogsRequestType {
  std::string owner_id;
...
struct RdsDescribeSlowLogsSQLSlowLogType {
  long slow_log_id;
  long sq_lid;
  std::string db_name;
  std::string sq_ltext;
...
struct RdsDescribeSlowLogsResponseType {
  std::vector<RdsDescribeSlowLogsSQLSlowLogType> items;
  std::string engine;
  std::string start_time;
  std::string end_time;
