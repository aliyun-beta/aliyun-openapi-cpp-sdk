...
void Json2Type(const Json::Value& value, RdsDescribePreCheckResultsResponseType* item);
...
void Json2Type(const Json::Value& value, RdsDescribePreCheckResultsResponseType* item) {
  ...
  if(value.isMember("Items") && value["Items"].isMember("PreCheckResult")) {
    Json2Array<RdsDescribePreCheckResultsPreCheckResultType>(value["Items"]["PreCheckResult"], &item->items);
  }
}
}
